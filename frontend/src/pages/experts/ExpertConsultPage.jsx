import { Crown, Lock, MessageSquarePlus } from 'lucide-react'
      ...
      <main className={styles.main}>
        <div className={styles.headingRow}>
          ...
          {isExpert && (
            <button
              type="button"
              className={`${styles.modeToggle} ${expertMode ? styles.modeToggleActive : ''}`}
              onClick={() => setExpertMode((prev) => !prev)}
            >
              <span className={`${styles.switchTrack} ${expertMode ? styles.switchTrackActive : ''}`}>
                <span className={`${styles.switchThumb} ${expertMode ? styles.switchThumbActive : ''}`} />
              </span>
              전문가 모드
            </button>
          )}
