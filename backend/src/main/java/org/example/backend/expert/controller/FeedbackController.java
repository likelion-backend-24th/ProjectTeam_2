...
public class FeedbackController {
    ...
    @Operation(
            summary = "F-30 전문가 피드백 요청 등록",
            description = "구독자가 담당 전문가를 지정해 질문 스레드를 개설합니다. 비구독자 403, 미승인 전문가 지정 시 403."
    )
    ...
    @Operation(summary = "F-30 피드백 스레드 상세 조회", description = "스레드 상태(PENDING/ANSWERED)와 기본 정보를 조회합니다.")
    ...
    @Operation(
            summary = "F-30 피드백 메시지 등록",
            description = "스레드에 메시지를 추가합니다. 담당 전문가 답변 시 ANSWERED 전환. 요청자/담당 전문가 아니면 403."
    )
    ...
    @Operation(
            summary = "F-30 내 문의 스레드 목록 조회 (구독자용)",
            description = "로그인한 구독자 본인이 개설한 문의 스레드 목록. 전문가 닉네임·상태·답변시각 포함."
    )
    ...
    // 참고: 명세서 F-30 "관련 API" 목록엔 없는 엔드포인트입니다. 화면 설계(S-11)에 EXPERT용 대칭 화면
    // 언급은 있으나 화면-API 매핑 표엔 없어 팀 확인이 필요합니다. 확인 전까지는 유지만 해둡니다.
    @Operation(
            summary = "받은 문의 목록 조회 (전문가용, 명세서 미확정)",
            description = "로그인한 유저가 전문가로서 받은 문의 스레드 목록. 명세서 API 매핑 표에는 없어 팀 확인 필요."
    )
