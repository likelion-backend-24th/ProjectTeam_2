...
public class StudyPostService {
    ...
    public List<StudyPostResponse> getStudyPosts(Long userId, Long id) {
        Study study = studyRepository.findById(id).orElseThrow(StudyNotFoundException::new);
        studyMemberRepository.findByStudyIdAndUserId(id, userId).orElseThrow(StudyMemberNotFoundException::new);
    ...
    public StudyPostResponse getStudyPost(Long userId, Long id, Long postId) {
        Study study = studyRepository.findById(id).orElseThrow(StudyNotFoundException::new);
        studyMemberRepository.findByStudyIdAndUserId(id, userId).orElseThrow(StudyMemberNotFoundException::new);
        StudyPost post = studyPostRepository.findById(postId).orElseThrow(StudyPostNotFoundException::new);

        if (!post.getStudy().getId().equals(id)) {
            throw new StudyPostMismatchException();
        }
    ...
    public StudyPostResponse updateStudyPost(Long userId, Long id, Long postId, StudyPostUpdateRequest request) {
        ...
        if (!post.getUser().getId().equals(userId)) {
            throw new StudyPostForbiddenException();
        }
